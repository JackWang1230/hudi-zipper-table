...
//            druidDataSource.setUseUnfairLock(true);
    ...
    public static Connection getConnection(ParameterTool tool) throws SQLException {
        parameterTool =  tool;
        connection =
         JDBCDruidUtils.getConnection(parameterTool.get(PropertiesConstants.MYSQL_DATABASE_URL),parameterTool.get(PropertiesConstants.MYSQL_DATABASE_USER),
                parameterTool.get(PropertiesConstants.MYSQL_DATABASE_PASSWORD),parameterTool.getInt(PropertiesConstants.MYSQL_DATABASE_INITIAL_SIZE),
                parameterTool.getInt(PropertiesConstants.MYSQL_DATABASE_MIN_IDLE),parameterTool.getInt(PropertiesConstants.MYSQL_DATABASE_MAX_ACTIVE),
                parameterTool.getInt(PropertiesConstants.MYSQL_DATABASE_MAX_WAIT),parameterTool.getLong(PropertiesConstants.MYSQL_DATABASE_TIME_BETWEEN_EVICTABLE_IDLE),
                parameterTool.getLong(PropertiesConstants.MYSQL_DATABASE_MAX_EVICTABLE_IDLE),parameterTool.getLong(PropertiesConstants.MYSQL_DATABASE_MIN_EVICTABLE_IDLE),
                parameterTool.getBoolean(PropertiesConstants.MYSQL_DATABASE_TEST_WHILE_IDLE),parameterTool.getBoolean(PropertiesConstants.MYSQL_DATABASE_TEST_ON_BORROW),
                parameterTool.getBoolean(PropertiesConstants.MYSQL_DATABASE_TEST_ON_RETURN),parameterTool.get(PropertiesConstants.MYSQL_DATABASE_VALIDATION_QUERY));
        return connection;
    }
